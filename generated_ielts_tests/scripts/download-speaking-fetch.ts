...
/**
 * Download Engnovate IELTS General Training Speaking pages into speaking/<id>/fetch.html
 * Usage:
 *   npx --yes tsx generated_ielts_tests/scripts/download-speaking-fetch.ts generated_ielts_tests/speaking/18-4 [...more]
 */
import fs from "fs";
...
function buildUrl(vol: string, test: string): string {
  return `https://engnovate.com/ielts-speaking-tests/cambridge-ielts-${vol}-general-training-speaking-test-${test}/`;
}
...
async function main(): Promise<void> {
  ...
  if (targets.length === 0) {
    console.error(
      "Usage: npx --yes tsx generated_ielts_tests/scripts/download-speaking-fetch.ts generated_ielts_tests/speaking/<vol>-<test> [...more]",
    );
    process.exit(1);
  }
