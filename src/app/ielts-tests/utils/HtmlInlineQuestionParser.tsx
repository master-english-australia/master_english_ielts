...
export const HtmlInlineQuestionParser: React.FC<{ htmlText: string, questions: Question[] }> = ({ htmlText, questions }) => {
  let fieldIndex = 0;
    ...
    replace: (domNode) => {
      if (domNode.type === 'text' && domNode.data?.includes('______')) {
        ...
        for (let i = 0; i < parts.length; i++) {
          ...
          if (i < parts.length - 1) {
            nodes.push(
              <Box sx={{ display: 'inline-flex', alignItems: 'center', gap: 1, mx: '2px', my:'2px' }}>
